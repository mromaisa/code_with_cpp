...
public:
    void Graph_form(const string &task, int pri, int time, const vector<string> &dependencies)
    {
    ...
    vector<string> topological_sort()
    {
        ...
        priority_queue<pair<string, int>, vector<pair<string, int>>, ComparePriority> pq;
        ...
        for (const auto &entry : dependency_count)
        {
            if (entry.second == 0)
            {
                pq.push({entry.first, priority[entry.first]});
            }
        }

        while (!pq.empty())
        {
            string task = pq.top().first;
            pq.pop();
            ...
            for (const auto &neighbour : adj_list[task])
            {
                ...
                {
                    pq.push({neighbour, priority[neighbour]});
                }
            }
        }
...
int main()
{
    ...
    while (true)
    {
        ...
        cout << "\nEnter priority of this task(1 - 4): ";
        int pri;
        cin >> pri;
        if (pri < 1 || pri > 4)
        {
            cout << "Invalid priority! It should be between 1 and 4 so default value of 4 will be assigned.\n";
            pri = 4;
        }
        cin.ignore();
        ...
        cout << "\nEnter dependencies (one per line, enter 'e' to stop):\n";
        while (true)
        {
            getline(cin, dep);
            if (dep == "e")
        ...
        g.Graph_form(task, pri, time, dependencies);
    }
